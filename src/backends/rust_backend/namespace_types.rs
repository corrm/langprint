...
use super::{
    ...
    RustEnumConversionOptions, RustEnumRenderOptions, RustFunction, RustFunctionConversionOptions,
    RustFunctionRenderOptions, RustStruct, RustStructConversionOptions, RustStructRenderOptions,
    RustVisibility,
};

/// Convert an optional list of native items to their IR form, collecting any warnings.
fn module_items_to_ir<T: BackendItem>(
    items: Option<Vec<T>>,
...
impl BackendItem for RustModule {
    ...
    fn from_ir(
        ...
        let struct_options = RustStructConversionOptions {
            config: config.clone(),
        };
        let function_options = RustFunctionConversionOptions {
            config: config.clone(),
        };
        let module_options = RustModuleConversionOptions {
            config: config.clone(),
        };
        ...
        let rust_module = RustModule {
            ...
            structs: module_items_from_ir::<RustStruct>(
                input.structs,
                Some(&struct_options),
                &mut log,
            ),
            functions: module_items_from_ir::<RustFunction>(
                input.functions,
                Some(&function_options),
                &mut log,
            ),
            modules: module_items_from_ir::<RustModule>(
                input.namespaces,
                Some(&module_options),
                &mut log,
            ),
